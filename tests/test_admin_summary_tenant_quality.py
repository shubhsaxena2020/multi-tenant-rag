...
def test_admin_summary_tenant_data_quality():
    ...
    assert d["tenant_count"] > 0, f"Expected positive tenant_count, got {d['tenant_count']}"
    
    # Verify not ALL tenants have zero activity (some should have data)
    active_tenants = [t for t in d["tenants"] if t.get("queries", 0) > 0 or t.get("docs_ingested", 0) > 0]
    ...
    expected_fields = ["tenant_id", "name", "plan", "queries", "docs_ingested", "chunks_ingested"]
