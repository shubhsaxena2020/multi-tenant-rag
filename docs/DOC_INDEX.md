...
|| `qdrant_unhealthy_snapshot_restore_runbook.md` | Qdrant unhealthy diagnosis and snapshot restore runbook with copy-paste commands and code-path references to `app/backup.py` and `app/vector_store.py` | Fresh (validated against live service: health checks, circuit breakers, create/list/recover snapshot flow) |
