...
When operators follow these steps, **tenant creation fails with `403 "Admin key required"`** because the `ADMIN_API_KEY` environment variable is not loaded into the app container, even though `.env` contains `ADMIN_API_KEY=admin_master_key`.
...
|---|---|---|
| Tenant create | `Admin-Key: admin_master_key` | 201 Tenant created |
| Key create | `Admin-Key: admin_master_key` | 201 Key issued |
...
- [ ] Key create: `curl -s -X POST ... -H "Admin-Key: admin_master_key"` → 201
...
- `README.md` line 63 — `ADMIN_API_KEY=admin_master_key` (corrected from `***`)
