...
New operators frequently attempt to use the `Admin-Key: <your-admin-key>` header on query endpoints, or use `Authorization: Bearer <publishable_key>` on admin endpoints. Both patterns fail with `403 {"detail":"Admin key required"}` or `401 {"detail":"Invalid API key"}`, respectively.

**The rule of thumb:**
- `Admin-Key: <your-admin-key>` → admin operations (tenant/create, key rotation, revocation)
...
# Expected: ADMIN_API_KEY=<your-admin-key>
...
**Authentication: `Admin-Key: <your-admin-key>` header**
...
**Common mistake:** Using `Authorization: Bearer ***` instead of `Admin-Key: <your-admin-key>` → returns `403 {"detail":"Admin key required"}`
...
**Common mistake #1:** Using `Admin-Key: <your-admin-key>` on query endpoint → `403 {"detail":"Admin key required"}`
...
|---|---|---|---|
| Tenant create | `Admin-Key: <your-admin-key>` | `curl -s -X POST ... -H "Admin-Key: <your-admin-key>"` | `403 Admin key required` |
| Key create | `Admin-Key: <your-admin-key>` | `curl -s -X POST ... -H "Admin-Key: <your-admin-key>"` | `403 Admin key required` |
...
- [ ] `.env` has `ADMIN_API_KEY=<your-admin-key>` (not `***`)
- [ ] Service health: `curl -s http://localhost:8000/health` → 200
- [ ] Tenant create: `curl -s -X POST ... -H "Admin-Key: <your-admin-key>"` → 201
...
- Admin operations always use `Admin-Key: <your-admin-key>`
