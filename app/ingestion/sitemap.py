"""Sitemap.xml crawler for client onboarding (PHASE B.2).

A client points the service at their sitemap.xml and we ingest every page as a
document. Designed to be a *respectful* crawler:
...
Crawling is synchronous inside a worker thread (driven by the async job runner); the
HTTP endpoint returns a job_id immediately so large sites don't block the request.
"""
...
def parse_sitemap(xml_text: str) -> tuple[list[str], list[str]]:
    ...
    for loc in _loc_tags(root):
        # Heuristic: a child sitemap path often ends in .xml and we differentiate by root tag.
        if root_tag == "sitemapindex":
            children.append(loc)
        else:
            pages.append(loc)
    # Fallback if namespace stripped everything to a bare 'sitemapindex' detection failed:
    if not children and root_tag == "sitemapindex":
        pass
    return pages, children


def _robots_rules(robots_text: str) -> tuple[set[str], float]:
    """Parse a robots.txt body into (allowed_prefixes, crawl_delay).
...
def _compile_robots(allows: list[str], disallows: list[str]) -> set[str]:
    ...
    # Allow overrides: remove prefixes that are explicitly allowed.
    for a in allows:
...
async def crawl_sitemap(
    ...
    *,
    ...
    Idempotent per URL via source_hash (SHA-256 of the URL) so a re-crawl REPLACES the
    prior doc for that URL instead of duplicating it (PHASE B.1). Respects robots.txt.
    """
    import hashlib
    ...
    async def _ingest_one(url: str) -> None:
        ...
        if _is_disallowed(path, disallowed):
            ...
            return
        try:
            # source_hash keyed on the URL => idempotent re-crawl replaces the doc.
            src_hash = hashlib.sha256(url.encode()).hexdigest()
            await ingest_url(
                tenant_id, url, title=url, metadata=metadata,
                acl=acl, source_hash=src_hash,
            )
